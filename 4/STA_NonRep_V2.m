...
for iNeuron=1:NEURONS
    ...
    for iAP=1:NUM_OF_APS %for every AP
        ...
        timeOfNonRepWindow = Simulation.StimTimeNonRep(idxNonRepWindow);
        timeOfNonRepNextWindow = Simulation.StimTimeNonRep(idxNonRepWindow+1);
        ...
        if (apWindowStims(1,1)<timeOfNonRepWindow) %discard stims before non rep window starts
            continue;
        end
        ...
        if (apBin(end,1)>=timeOfNonRepNextWindow) %discard stims after non rep window ends
            %go to next non window if we're after it
            idxNonRepWindow = idxNonRepWindow+1;
            continue;
        end
